...
const RepetationComponent = () => {
  ...
  const getDetail = async () => {
    try {
      ...
      const {_data:{days}}=moment.duration(day, 'seconds');
      setDays(days);
    } catch (e) {
      console.log(e);
    }
        ...
        <div className="col-lg-8">
          <div className="card stacking_card">
            <div className="d-flex stact_inner">
              ...
              <div>
                <p className='stack_p width_adjust'>Contract address: <a href={`https://legacy.wyzthscan.org/address/${financeAppContractAddress}`} className="stack_p" target="_blank">{financeAppContractAddress} </a></p>
                <p className='stack_p width_adjust2'>Contract address: <a href={`https://legacy.wyzthscan.org/address/${financeAppContractAddress}`} className="stack_p" target="_blank" >{(financeAppContractAddress?.substring(0, 4) + "..." + financeAppContractAddress?.substring(financeAppContractAddress?.length - 4))}</a></p>
